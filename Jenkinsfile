def linuxBuildBadge = addEmbeddableBadgeConfiguration(id: "linuxbuild", subject: "Pipeline Status")

pipeline {
  environment {
    GIT_URL = "git@150.129.147.154:web/node/edulytic-nodejs.git"
    PROJECT_NAME = "edulytic-solution-api-test"
    PROJECT_SUFFIX = "web"
    ...
    PROJECT_PATH = "/home/developer/node-projects/php-projects"
    ...
    recipients_email="devops@codiant.com"
  }
  agent any
  ...
  tools {nodejs "NodeJs_16"}
   stages {
    ...
    stage('Badges Status') {
        steps {
            script {
                linuxBuildBadge.setStatus('running')
                 try {
                     linuxBuildBadge.setStatus('Success')
                    } catch (Exception err) {
                     linuxBuildBadge.setStatus('Failed')
                    }
                }
        }
    }
      ...
      failure {
 	    script {
   	        emailext body:"""
                    <html>
  		            <body>
     			          <p style="color: red; font-weight: bold;">Deployment Failed!</p>
     			          <p>Description :Deployment Failed on Environment:${env.JOB_NAME}</p>
     			          <p>Build Number: ${currentBuild.number}</p>
    			          <p>Check Console Output at URL: <a href="${env.BUILD_URL}">${env.BUILD_URL}</a></p>
                  	      <p>Build Log is attached.</p>
     			          <p>- Codiant DevOps Team</p>
  		            </body>
		            </html>
                    """,
                    subject: "Jenkins Pipeline: ${env.JOB_NAME} Failed",
                    to: "${env.recipients_email}",
                    mimeType: 'text/html',
                    attachLog: true
  		    }
	    }
